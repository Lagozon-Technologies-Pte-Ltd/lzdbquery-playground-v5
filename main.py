...
@app.post("/run-ingest-metadata")
async def run_ingest_metadata():
    try:
        # Run the script and capture output
        result = subprocess.run(
            ["python", "IngestMetadata.py"],
            capture_output=True,
            text=True
        )
...
@app.post("/run-ingest-examples")
async def run_ingest_examples():
    try:
        # Run the script and capture output
        result = subprocess.run(
            ["python", "IngestExamples.py"],
            capture_output=True,
            text=True
        )
...
async def submit_query(
    ...
    try:
       # **Step 1: Invoke Unified Prompt**
        ...
        if fetch_semantic_schema:
            table_schema, column_schema = get_table_and_column_schema(llm_reframed_query)
