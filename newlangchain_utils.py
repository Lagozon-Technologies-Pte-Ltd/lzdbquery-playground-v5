...
def get_chain(question, selected_database, table_details, selected_business_rule,question_type,relationships,table_schema,column_schema,examples):
...
# )
    ...
    if question_type == "generic":
        ...
        print("entered in get_chain...")
        final_prompt = static_prompt.format(
        ...
        #     ]
        # )
        final_prompt = static_prompt.format(table_info=table_details, Business_Rule = selected_business_rule, Business_Glossary = business_glossary, relationships=relationships_str,table_schema=table_schema,column_schema=column_schema)
    print("langchain prompt: ", final_prompt)
    # if selected_database=="GCP":
    #         db = BigQuerySQLDatabase()
    # elif selected_database=="PostgreSQL-Azure":  #####commented by Rohit on 3 july
...
# --- LLM Invocation with JSON Mode ---
    ...
    return json_output, final_prompt



def invoke_chain(db,question, messages, selected_model, selected_subject, selected_database, table_info,selected_business_rule,question_type,relationships,table_schema,column_schema,examples):
    ...
    try:
        ...
        if selected_database == "Azure SQL":
            ...
            # db = get_sql_db(selected_subject, mahindra_tables)
            result = db.execute(query)  # SQLAlchemy ResultProxy
